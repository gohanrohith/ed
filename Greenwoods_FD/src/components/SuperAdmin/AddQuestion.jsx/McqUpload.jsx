...
const McqUpload = ({ chapterId, questionType, chapterName }) => {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    // Retrieve userId from localStorage and get adminId from the API
  ...
  <div style={{ overflowX: 'auto', width: '100%' }}> {/* Container for horizontal scroll */}
    <table style={{ 
      width: '100%', 
      borderCollapse: 'collapse',
      borderRadius: '8px',
      overflow: 'hidden',
      minWidth: '800px' // Ensure the table has a minimum width to force scrolling
    }}>
      <thead>
        <tr style={{ 
          ...
          {['Question', 'OptionA', 'OptionAImage', 'OptionB', 'OptionBImage', 'OptionC', 'OptionCImage', 'OptionD', 'OptionDImage', 'CorrectAnswer'].map((h) => (
            <th key={h} style={{ 
              padding: '12px', 
              border: '1px solid #e8e8e8',
              fontWeight: 600,
              whiteSpace: 'nowrap', // Prevent text wrapping
              minWidth: '120px' // Set a minimum width for each header cell
            }}>
              {h}
            </th>
          ))}
      ...
      <tbody>
        <tr>
          ...
          {['Berlin', '', 'Paris', '', 'Madrid', '', 'Rome', '', 'B'].map((c, i) => (
            <td 
              key={i} 
              style={{ 
                padding: '12px', 
